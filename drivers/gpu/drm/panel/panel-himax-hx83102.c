...
{
    int ret;
    /* Initial panel stabilization delay */
    msleep(60);

    /* Complete ProductShop HX83102E initialization sequence */
    ...
    mipi_dsi_dcs_write_seq(dsi,0xBD,0x00);
    mipi_dsi_dcs_write_seq(dsi,0xD8,0xAA,0xAA,0xEA,0xAA,0xAA,0xA0,0xAA,0xAA,0xEA,0xAA,0xAA,0xA0);
    mipi_dsi_dcs_write_seq(dsi,0xBD,0x01);
    mipi_dsi_dcs_write_seq(dsi,0xD8,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00);
    mipi_dsi_dcs_write_seq(dsi,0xBD,0x02);
    mipi_dsi_dcs_write_seq(dsi,0xD8,0xFF,0xFF,0xFF,0xFF,0xFF,0xF0,0xFF,0xFF,0xFF,0xFF,0xFF,0xF0);
    mipi_dsi_dcs_write_seq(dsi,0xBD,0x03);
    mipi_dsi_dcs_write_seq(dsi,0xD8,0x55,0x55,0x55,0x55,0x55,0x50,0x55,0x55,0x55,0x55,0x55,0x50,0x55,0x55,0x55,0x55,0x55,0x50,0x55,0x55,0x55,0x55,0x55,0x50);
    mipi_dsi_dcs_write_seq(dsi,0xBD,0x00);
    ...
    mipi_dsi_dcs_write_seq(dsi,0xBD,0x00);
    mipi_dsi_dcs_write_seq(dsi,0xCB,0x00,0x13,0x08,0x02,0x12);
    mipi_dsi_dcs_write_seq(dsi,0xBD,0x02);
    mipi_dsi_dcs_write_seq(dsi,0xCB,0x00,0x03,0x00,0x01,0x70);
    mipi_dsi_dcs_write_seq(dsi,0xBD,0x00);
    mipi_dsi_dcs_write_seq(dsi,0xC7,0x30,0xB0,0xC2,0x08,0x10,0x42,0x04);
    mipi_dsi_dcs_write_seq(dsi,0xD1,0x67,0x0C,0x0F);
    mipi_dsi_dcs_write_seq(dsi,0xE1,0x00,0x04);
    
    // ❌ FIXED: Remove extra 0x00 byte - should match reference exactly
    mipi_dsi_dcs_write_seq(dsi,0xB9,0x00,0x00,0x00);  // Was: 0x00,0x00,0x00,0x00
    ...
    // ❌ FIXED: Use 800ms delay like reference (was 120ms)
    msleep(800);  

    ret = mipi_dsi_dcs_set_display_on(dsi);
    if (ret < 0) return ret;
    
    // ❌ FIXED: Use 120ms delay like reference (was 20ms)  
    msleep(120);

    return 0;
}
static const struct drm_display_mode productshop_mode = {
    .clock = 171000,        // Was 158000 - now 171 MHz
    .hdisplay = 1200,
    .hsync_start = 1200 + 24,   // HFP = 24 (was 80)
    .hsync_end = 1200 + 24 + 12, // Hsync = 12 (was 24)  
    .htotal = 1200 + 24 + 12 + 24, // HBP = 24 (was 80)
    .vdisplay = 2000,
    .vsync_start = 2000 + 224,  // VFP = 224 (was 20)
    .vsync_end = 2000 + 224 + 12, // Vsync = 12 (was 4)
    .vtotal = 2000 + 224 + 12 + 28, // VBP = 28 (was 20)
    .width_mm = 142,        // Keep your physical dimensions
    .height_mm = 237,
...
static int hx83102_prepare(struct drm_panel *panel)
{
    ...
    dev_err(ctx->dev, "*** HX83102 PREPARE FUNCTION CALLED ***\n");

    if (ctx->prepared) {
        dev_err(ctx->dev, "Panel already prepared, skipping\n");
        return 0;
    }
    ...
    /* Reset sequence for proper panel initialization - HX83102 requires 120ms high */
    if (ctx->reset_gpio) {
        dev_info(ctx->dev, "=== STARTING RESET SEQUENCE ===\n");
        
        /* Ensure reset is HIGH initially (normal operation) */
        gpiod_set_value_cansleep(ctx->reset_gpio, 1);  // HIGH - normal operation
        dev_info(ctx->dev, "Reset pin HIGH (normal), waiting 10ms\n");
        msleep(200);
        
        /* Assert reset by pulling LOW for 120 (RESET) */
        gpiod_set_value_cansleep(ctx->reset_gpio, 0);  // LOW - asserting reset
        dev_info(ctx->dev, "Reset asserted (LOW), holding for 10ms\n");
        msleep(120);  // Hold reset LOW (well above 15μs minimum)
        
        /* De-assert reset by returning to HIGH */
        gpiod_set_value_cansleep(ctx->reset_gpio, 1);  // HIGH - de-asserting reset
        dev_info(ctx->dev, "Reset de-asserted (HIGH), waiting 120ms for panel to complete reset\n");
        msleep(10);  // Wait for tREST (max 120ms for reset complete)
        
        dev_info(ctx->dev, "=== RESET SEQUENCE COMPLETE ===\n");

        /* Reset pin stays HIGH for normal operation */
    } else {
        dev_err(ctx->dev, "ERROR: No reset GPIO available!\n");
    }
    dev_info(ctx->dev, "Starting panel initialization sequence\n");
    ret = ctx->desc->init(ctx->dsi);
    if (ret < 0) {
        dev_err(ctx->dev, "Panel init failed: %d\n", ret);
        goto poweroff;
    }
    dev_info(ctx->dev, "Panel initialization completed successfully\n");
...
static int hx83102_probe(struct mipi_dsi_device *dsi)
{
    ...
    dev_info(dev, "HX83102 panel probe starting\n");
    ...
    dev_info(dev, "Attempting to acquire reset GPIO\n");
    ctx->reset_gpio = devm_gpiod_get(dev, "reset", GPIOD_OUT_HIGH);
    if (IS_ERR(ctx->reset_gpio)) {
        dev_err(dev, "Failed to get reset GPIO: %ld\n", PTR_ERR(ctx->reset_gpio));
        ctx->reset_gpio = NULL;
    } else {
        dev_info(dev, "Successfully acquired reset GPIO\n");
    }
    ...
    dev_info(dev, "Attempting to attach to DSI host\n");
    ...
    dev_info(dev, "Successfully attached to DSI host\n");
    return 0;
}
