...
//mipi_dsi_dcs_write_seq(dsi,0xCF,0xFF);
msleep(20);
...
static int hx83102_prepare(struct drm_panel *panel)
{
    ...
    if (ctx->reset_gpio) {
        dev_err(ctx->dev, "=== STARTING RESET SEQUENCE ===\n");
        gpiod_set_value_cansleep(ctx->reset_gpio, 0);
        dev_err(ctx->dev, "Reset set LOW, waiting 20ms\n");
        msleep(20);
        dev_err(ctx->dev, "Setting reset HIGH for 120ms\n");
        gpiod_set_value_cansleep(ctx->reset_gpio, 1);
        msleep(120);
        dev_err(ctx->dev, "=== RESET SEQUENCE COMPLETE ===\n");
    } else {
