	...
	const struct firmware *file_entry, int tx_num, int rx_num)
{
	...
	if (g_inspt_crtra[crtra_id] != NULL) {
		E("%s,%d: Memory allocation %d already exist!\n",
			__func__, __LINE__, crtra_id);
		err = HX_INSP_MEMALLCTFAIL;
		goto err_malloc_inspection_criteria;
	}
	g_inspt_crtra[crtra_id] = kcalloc((hx_s_ic_data->tx_num
				* hx_s_ic_data->rx_num),
				sizeof(int), GFP_KERNEL);
	if (g_inspt_crtra[crtra_id] == NULL) {
	...
	I("%s:g_inspt_crtra[%d]=%s\n", __func__,
		crtra_id,
		g_hx_inspt_crtra_name[crtra_id]);
...
static int himax_self_test_data_init(void)
{
	...
	if (err < 0) {
		...
		count_item = himax_test_item_chk(true);
		if (count_item == 0) {
	...
	if (g_rslt_data == NULL) {
		...
		g_rslt_data = vmalloc(
			g_1kind_raw_size * count_item * sizeof(char)
			);
		if (g_rslt_data == NULL) {
			E("%s,%d: Memory allocation falied!\n",
				__func__, __LINE__);
		...
		memset(g_rslt_data, 0x00,
			g_1kind_raw_size * count_item);
	}

	I("%s: initialize g_rslt_data, length = %d\n",
		__func__, g_1kind_raw_size * count_item);
	memset(g_rslt_data, 0x00,
		g_1kind_raw_size * count_item * sizeof(char));
