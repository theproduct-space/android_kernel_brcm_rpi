...
int smcu_done_disable_irq(struct nfc_dev *dev)
{
	struct nfc_dev *nfc_dev = dev;
	unsigned long flags = 0;
	spin_lock_irqsave(&nfc_dev->smcu_irq_enabled_lock, flags);
	if (!nfc_dev->smcu_irq_enabled) {
		disable_irq_nosync(nfc_dev->irq_sw_smcu_done);
		nfc_dev->smcu_irq_enabled = false;
	}
	spin_unlock_irqrestore(&nfc_dev->smcu_irq_enabled_lock, flags);
	return 0;
}

int smcu_done_enable_irq(struct nfc_dev *dev)
{
	struct nfc_dev *nfc_dev = dev;
	unsigned long flags = 0;
	spin_lock_irqsave(&nfc_dev->smcu_irq_enabled_lock, flags);
	if (!nfc_dev->smcu_irq_enabled) {
		nfc_dev->smcu_irq_enabled = true;
		enable_irq(nfc_dev->irq_sw_smcu_done);
	}
	spin_unlock_irqrestore(&nfc_dev->smcu_irq_enabled_lock, flags);
	return 0;
}
...
{
	struct nfc_dev *nfc_dev = dev_id;
	nfc_dev->smcu_fw_dnld_triggered = true;
	...
	wake_up(&nfc_dev->smcu_fw_dnld_wq);
	return IRQ_HANDLED;
}
...
int nfc_i2c_dev_probe(struct i2c_client *client)
{
	...
	init_waitqueue_head(&nfc_dev->smcu_fw_dnld_wq);
	...
	spin_lock_init(&nfc_dev->smcu_irq_enabled_lock);
